if(controllable){
	...
	if(attack_check && action == "none"){
		...
		if(!grounded){
			if(vspeed > -(jumpPower / 2) && vspeed < jumpPower / 2){
				action = "divekick";
				sprite_index = Spr_Jenny_DiveKick;
				image_index = 0;
		...
		// Ducking
			...
			action = "uppercut";
			sprite_index = Spr_Jenny_Uppercut;
			image_index = 0;
	...
	if(ability_check && abilityReady && action == "none" && grounded){
		action = "ability";
		sprite_index = Spr_Jenny_Ability;
		image_index = 0;
		...
		friction = 4;
		hspeed = -48 * image_xscale;
	}
	
	if(sprite_index = Spr_Jenny_Dance && dancePower < 240){ // 240 is max amount of dance power
		dancePower += 1;
	}
}