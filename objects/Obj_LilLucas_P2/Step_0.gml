if(controllable){
	...
	// Lil Lucas aerial drift
	if(!grounded){
		if(right_check && !left_check && hspeed < ms){
			hspeed += 2;
		}
		else if(left_check && !right_check  && hspeed > -ms){
			hspeed -= 2;
		}
	}
	...
	if(attack_check && action == "none"){
		...
		if(!grounded){
			if(vspeed > -(jumpPower / 2) && vspeed < jumpPower / 2){
				action = "divekick";
				sprite_index = Spr_LilLucas_DiveKick;
				image_index = 0;
		...
		// Ducking
			...
			action = "uppercut";
			sprite_index = Spr_LilLucas_Uppercut;
			image_index = 0;
		...
		// Standing
			...
			action = "kick";
			sprite_index = Spr_LilLucas_Kick;
			image_index = 0;
	...
	if(ability_check && abilityReady && action == "none" && grounded){
		action = "ability";
		sprite_index = Spr_LilLucas_Ability;
		image_index = 0;
